package code;
import java.io.*;//命令编译器进入io路径下所有的类
import java.util.ArrayList;//引入动态数组
import java.util.List;//引用在java包下的util包下的list类 属于接口借用此接口方法必须使用子类Arrylist实现操作
import java.util.Scanner;//引入Scannner类可以通过Scanner类来获取用户的输入

public class AscllSort3 { //定义公共类 AscllSort3
    //内部的每个List都是一个单独的省会信息
    //第一个位置就是存储着省份名称
    private static final List<List<String>> province = new ArrayList<>();//定义一个静态私有final类作为省份的动态数组
    ...
    private static void readFileByInFileName(String fileName) {//以某种单位读取文件
        //读取文件
        try (BufferedReader reader = new BufferedReader(//尝试使用新的数据=旧的文件
